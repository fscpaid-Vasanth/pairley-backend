...
export class AuthService implements OnModuleInit {
  ...
  async verifyOtp(mobile: string, code: string) {
    ...
    const business = await this.prisma.business.findUnique({ where: { mobile } });

    if (customer) {
      const token = this.generateToken(customer.id, customer.mobile, 'Customer');
      return { exists: true, role: 'Customer', token, user: customer };
    }

    if (business) {
      const token = this.generateToken(business.id, business.mobile, 'Business');
      return { exists: true, role: 'Business', token, user: business };
    }

    return { exists: false, message: 'OTP verified successfully. Please proceed to register.' };
  }
  ...
  async register(data: any) {
    ...
    if (role === 'Customer') {
      const existingCustomer = await this.prisma.customer.findUnique({ where: { mobile } });
      if (existingCustomer) {
        throw new BadRequestException('Customer already registered with this mobile number');
      }
      ...
      const customer = await this.prisma.customer.create({
        data: {
          ...
          date_of_birth: extra.date_of_birth ? new Date(extra.date_of_birth) : null,
      ...
      const existingBusiness = await this.prisma.business.findUnique({ where: { mobile } });
      if (existingBusiness) {
        throw new BadRequestException('Business owner already registered with this mobile number');
      }

      const passwordHash = extra.password ? await bcrypt.hash(extra.password, 10) : null;
  ...
  async googleUpsert(data: any) {
    ...
    if (role === 'Customer') {
      ...
      let business = await this.prisma.business.findUnique({ where: { mobile } });
  ...
  async login(email: string, password_hash: string) {
    ...
    if (business && business.password_hash) {
      const match = await bcrypt.compare(password_hash, business.password_hash);
      if (!match) {
  ...
  async updateProfile(userId: string, role: string, updates: any) {
    ...
    const { id, mobile, created_at, updated_at, verification_status, subscription_id, password_hash, ...validUpdates } = updates;
    ...
    if (role === 'Customer') {
      ...
      if (validUpdates.password) {
        validUpdates.password_hash = await bcrypt.hash(validUpdates.password, 10);
  ...
  async deleteAccount(userId: string, role: string) {
    if (role === 'Customer') {
      ...
      return { success: true, message: 'Business account deleted successfully' };
    } else if (role === 'Admin') {
  ...
  private generateToken(id: string, mobile: string, role: string, email?: string) {
